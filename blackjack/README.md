...
想說算了 應該有洞 沒看code 直接玩

就bet打 `999999999999999999999999` overflow 只賭 `1`

之後嘗試打 -`9999` 然後故意輸 就會得到$$了

因為  它直接 `cash=cash±bet`

而且 只有檢查 `bet>cash` 沒有檢查 `bet`是負數的時候

還有第二種解法 就是 `bet>cash`只會檢查一次

如果第一次輸入大於cash 它就會叫你輸入第二次
